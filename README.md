...
- 仅在数据库**不存在**（即首次启动）时创建。若库中已有 `admin`，只补齐角色与权限、**绝不覆盖密码**——升级或再次启动不会把你改过的密码打回默认值。
- 若你的 `data/ogc_users.db` 是旧的且 `admin` 已被设过密码，请继续使用你自己的密码登录。

普通用户在注册后默认拥有全部模块权限，管理员可在仪表盘中为每个用户单独配置：
